...
extract_data <- function(file, base_path = "~/data/data/campylobacteriosis/raw/2018") {
...
pdfs_to_read <- list.files("~/data/data/campylobacteriosis/raw/2018", "*.pdf")

out <- map_dfr(pdfs_to_read, extract_data)
write_csv(out, file.path("~/data/data/campylobacteriosis/raw/2018", "processed_pdfs.csv"))

# try 2017... Doesn't seem so flash...
